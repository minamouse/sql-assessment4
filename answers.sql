...
SELECT models.name AS model_name, brands.name AS brand_name, headquarters FROM models JOIN brands USING (brand_id) WHERE brands.name='Ford' AND models.name='Mustang';
...
SELECT COUNT(model_id) FROM models JOIN brands USING (brand_id) WHERE brands.name='Ford';
...
SElECT models.year, models.name, model_id, awards.name FROM models LEFT JOIN awards ON (model_id=winner_id) WHERE models.year >= 1964 ORDER BY -models.year;
...
SELECT brands.name, brands.founded FROM brands LEFT JOIN models USING (brand_id) WHERE model_id IS NULL;
